...
- `prototype/3d/src/main.js`：主编排和仍待进一步拆分的游戏主体，已抽出低风险核心模块、相机控制器、UI 控制器和狼 AI 控制器。
...
- `main.js` 已拆出低风险模块、水面反射 pass、玩家纯数据表、输入控制器、HUD/map 控制器、相机控制器和狼 AI 控制器，但玩家状态、战斗、姿态执行和主循环本地编排仍在主文件中。继续拆分时先抽边界清晰的控制器，不要直接大改战斗循环。
