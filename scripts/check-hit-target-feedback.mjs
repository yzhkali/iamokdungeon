...
assert(runtimeCombatJs.includes('import { createHitTargetFeedback } from "./hitTargetFeedback.js";'), 'runtime combat must import hit target feedback module');
assert(/const\s+\{\s*onHitTarget\s*\}\s*=\s*createHitTargetFeedback\s*\(\s*\{[\s\S]*spaceSlash[\s\S]*getDummies[\s\S]*getMonsters[\s\S]*sfx\s*:\s*SFX[\s\S]*boostImpact\s*:\s*\(\s*nextHitstop\s*,\s*nextShake\s*\)\s*=>\s*\{[\s\S]*runtimeState\.hitstop\s*=\s*Math\.max\s*\(\s*runtimeState\.hitstop,\s*nextHitstop\s*\)[\s\S]*runtimeState\.shake\s*=\s*Math\.max\s*\(\s*runtimeState\.shake,\s*nextShake\s*\)/.test(runtimeCombatJs), 'runtime combat should wire hit target feedback with live dependencies');
assert(/onHitTarget\s*,/.test(runtimeCombatJs), 'hit resolution should still receive onHitTarget');
