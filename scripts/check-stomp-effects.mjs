...
assert(mainJs.includes('import { STOMP_RADIUS } from "./combat/stompEffects.js";'), 'main.js must import STOMP_RADIUS for the named stomp radius alias');
assert(runtimeCombatJs.includes('import { createStompEffects } from "./stompEffects.js";'), 'runtime combat must import stomp effects module');
...
assert(/\bconst\s+stompEffects\s*=\s*createStompEffects\s*\(\s*\{[\s\S]*\bTHREE\b[\s\S]*\bscene\b[\s\S]*getPlayer[\s\S]*getHittables[\s\S]*getDummies[\s\S]*playStomp\s*:\s*\(\s*\)\s*=>\s*SFX\.stomp\(\)[\s\S]*boostImpact\s*:\s*\(\s*\)\s*=>\s*\{[\s\S]*runtimeState\.hitstop\s*=\s*Math\.max\s*\(\s*runtimeState\.hitstop,\s*0\.12\s*\)[\s\S]*runtimeState\.shake\s*=\s*Math\.max\s*\(\s*runtimeState\.shake,\s*0\.45\s*\)/.test(runtimeCombatJs), 'runtime combat must create stompEffects with live scene/player/target/SFX dependencies');
...
assert(/spaceSlash\.update\s*\(\s*dt\s*\)\s*;\s*stompEffects\.updateStomps\s*\(\s*dt\s*\)\s*;/.test(updateControllerJs), 'effect update order must keep stomp effects after space slash');
