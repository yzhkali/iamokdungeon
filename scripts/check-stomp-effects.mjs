...
assert(/\bconst\s+stompEffects\s*=\s*createStompEffects\s*\(\s*\{[\s\S]*\bTHREE\b[\s\S]*\bscene\b[\s\S]*getPlayer\s*:\s*\(\s*\)\s*=>\s*P[\s\S]*getHittables\s*:\s*\(\s*\)\s*=>\s*hittables[\s\S]*getDummies\s*:\s*\(\s*\)\s*=>\s*dummies[\s\S]*playStomp\s*:\s*\(\s*\)\s*=>\s*SFX\.stomp\(\)[\s\S]*boostImpact\s*:\s*\(\s*\)\s*=>\s*\{\s*runtimeState\.hitstop\s*=\s*Math\.max\s*\(\s*runtimeState\.hitstop\s*,\s*0\.12\s*\)\s*;\s*runtimeState\.shake\s*=\s*Math\.max\s*\(\s*runtimeState\.shake\s*,\s*0\.45\s*\)\s*;?\s*\}[\s\S]*\}\s*\)/.test(mainJs), 'main.js must create stompEffects with live scene/player/target/SFX dependencies');
