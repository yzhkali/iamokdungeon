...
assert(/swordTrail\.updateTrail\s*\(\s*dt\s*\)\s*;\s*swordBeam\.updateBeams\s*\(\s*dt\s*,\s*beamHitByBeam\s*\)\s*;\s*updateSpinRings\s*\(\s*dt\s*\)\s*;\s*spaceSlash\.update\s*\(\s*dt\s*\)\s*;\s*updateStomps\s*\(\s*dt\s*\)\s*;/.test(mainJs), 'effect update order must keep space slash after rings and before stomps');
